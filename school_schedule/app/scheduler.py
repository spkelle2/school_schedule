...
def create_schedule(file_path):
	...
	meetings = list(av.index)
	times = list(av.columns)
	...
	x = LpVariable.dicts('x', (meetings, times), lowBound=0, upBound=1,
	                     cat='Integer')
	...
	for i in meetings:
	    for j in times:
	        prob += x[i][j] <= av.loc[i, j], ''
	        
	# 3
	# Each meeting must occur the required amount of times
	for i in meetings:
	    prob += lpSum(x[i][j] for j in times) == req.loc[i, 'Quantity'], ''
